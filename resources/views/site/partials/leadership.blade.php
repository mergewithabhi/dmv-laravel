...
<article>
    @if($person->photo)
        @php
            $leaderThumb = $person->photo->url('thumb') ?: $person->photo->url();
            $leaderWeb = $person->photo->url('web') ?: $person->photo->url();
        @endphp
        <img
            class="leader-photo"
            src="{{ $leaderThumb }}"
            srcset="{{ $leaderThumb }} 480w, {{ $leaderWeb }} 1600w"
            sizes="(max-width: 680px) calc(100vw - 56px), 300px"
            alt="{{ $person->photo->alt_text }}"
            loading="lazy"
            decoding="async"
        >
    @else
        <div class="leader-photo media-placeholder"><span>Coach image</span></div>
    @endif
    <div class="leader-copy">
        <h3>Coach {{ $person->display_name }}</h3><strong>{{ $assignment->role }}</strong>
        <p>{{ $person->biography }}</p>
        @if ($personSocials->isNotEmpty())
            <div class="leader-socials" aria-label="{{ $person->display_name }} social links">
